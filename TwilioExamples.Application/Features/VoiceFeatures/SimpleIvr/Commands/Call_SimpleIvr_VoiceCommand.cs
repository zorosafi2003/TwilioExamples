...
{
    public class Call_SimpleIvr_VoiceCommand : IRequest<VoiceResponse>
    {
        public string From { get; set; }
        public UrlInfoChildOfIvrBaseModel NextActionUrl { get; set; }
        public Call_SimpleIvr_VoiceCommand(string from , UrlInfoChildOfIvrBaseModel nextActionUrl)
        {
            From = from;
            NextActionUrl = nextActionUrl;
        }
        public class Call_SimpleIvr_VoiceCommandHandler : IRequestHandler<Call_SimpleIvr_VoiceCommand, VoiceResponse>
        {
            private string audioDir;
            private readonly ITwilioHelperProvider _twilioHelperProvider;

            public Call_SimpleIvr_VoiceCommandHandler(ITwilioHelperProvider TwilioHelperProvider)
            {
                _twilioHelperProvider = TwilioHelperProvider;

                audioDir = Url.Combine("audio", "voice", "simpleIvr");
            }
            public Task<VoiceResponse> Handle(Call_SimpleIvr_VoiceCommand command, CancellationToken cancellationToken)
            {
