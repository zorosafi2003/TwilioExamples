...
{
    public class TwilioHelperProvider : ITwilioHelperProvider
    {
        ...
        public TwilioHelperProvider(IHttpContextAccessor HttpContextAccessor, LinkGenerator LinkGenerator, IHostingEnvironment HostingEnvironment,
        ...
        public Uri ReturnFunctionUrl(ReturnFunctionUrlModel model)
        {
            ...
            var values = new
            {
                area,
                digits = model.Digits,
                dto = model == null ? null : JsonConvert.SerializeObject(model.DtoModel)
            };
        ...
        public void ReturnAudioFile(CustomGather gather, string message, string audioFileName, string dir)
        {
            if (_hostingEnvironment.IsDevelopment())
            {
                gather.Say(message);
        ...
        private dynamic GetHostUrl()
        {
            ...
            var host = (request.Host.Value.Contains("localhost") ? ngRokUrl : request.Host.Value);
